...
$context = $contextFactory->createFromReflector(new ReflectionMethod('\My\Index\Classy', '__construct'));
```
...
```php
$contextFactory = new \phpDocumentor\Reflection\Types\ContextFactory();
$context = $contextFactory->createForNamespace('\My\Index', file_get_contents('My/Index/Classy.php'));
```
...
Because Classy is a Class in the current namespace its FQSEN will have the `My\Index` namespace and by calling the 
`resolve` method of the FQSEN Resolver you will receive an `Fqsen` object that refers to 
`\My\Index\Classy::otherFunction()`.
